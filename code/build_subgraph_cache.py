...
parser.add_argument(
    ...
    metavar="S",
    help="split task",
    choices=["random", "cold", "cluster", "unseen_drug", "unseen_target"],
)
parser.add_argument(
    "--hop",
    default=2,
...
parser.add_argument(
    ...
    type=str,
    choices=["nested", "flat"],
    help="nested: k-hop subgraph cache; flat: plain molecule graph cache (用于无子图消融)",
)
args = parser.parse_args()


def get_cache_path(split_file_name):
    mode = args.use_nested
...
def build_one_split(data_dir, split_file_name):
    ...
    cached_samples = build_cache_samples(
        df,
        max_drug_nodes=290,
        use_nested=(args.use_nested == "nested"),
        h=args.hop,
