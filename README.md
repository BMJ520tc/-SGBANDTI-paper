...
│   └── bindingdb/random
├── manuscript/               # 论文手稿（tex / pdf / bib，MDPI Computers 模板）
├── results/                  # 全部实验结果（单独存放）
│   ├── 00_实验结果汇总.md     # 论文表格数据（mean±std + 逐种子）
│   ├── 01_调参过程与结果.md   # 超参微调记录
│   ├── results_manifest.csv  # 唯一结果清单（全模型 mean±SD + 逐 seed + 来源，P0-2 证据冻结）
│   ├── 配对bootstrap结果.md   # 显著性（SGBANDTI vs DrugBAN，逐样本重算）
│   ├── 基线协议表.md          # 各基线代码来源/模态/种子/测试样本/AUPRC 口径
│   ├── 数据来源与处理记录.md  # 数据溯源（BindingDB←DrugBAN、BioSNAP←iNGNN-DTI，哈希验证）
